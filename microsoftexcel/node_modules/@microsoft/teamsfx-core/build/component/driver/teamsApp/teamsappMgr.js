...
class TeamsAppMgr {
    async ensureAppPackageFile(inputs) {
        ...
        if (!(await fs_extra_1.default.pathExists(inputs["package-file"]))) {
            return teamsfx_api_1.err(new common_1.FileNotFoundError("updateTeamsApp", inputs["package-file"]));
        }
    ...
    async readManifestFromZip(appPackagePath) {
        ...
        return teamsfx_api_1.err(new common_1.FileNotFoundError("readManifestFromZip", appPackagePath + ":" + constants_1.Constants.MANIFEST_FILE));
    }
            ...
            if (!inputs["env-file"]) {
                ...
                if (inputs.env) {
                    ...
                    if (envs.includes(inputs.env)) {
                        ...
                        return teamsfx_api_1.err(new common_1.FileNotFoundError("updateTeamsApp", path.join(envFolder, `.env.${inputs.env}`)));
                    }
                }
                else {
                    //env not provided
                    if (envs.length > 1) {
                        //need provide
                        return teamsfx_api_1.err(new common_1.MissingRequiredInputError("env", "updateTeamsApp"));
                    }
    ...
    async packageTeamsApp(inputs) {
        if (!inputs["manifest-file"]) {
            ...
            if (!(await fs_extra_1.default.pathExists(defaultManifestPath))) {
                return teamsfx_api_1.err(new common_1.MissingRequiredInputError("package-file/manifest-file", "updateTeamsApp"));
            }
            inputs["manifest-file"] = defaultManifestPath;
        }
        else {
            if (!(await fs_extra_1.default.pathExists(inputs["manifest-file"]))) {
                return teamsfx_api_1.err(new common_1.FileNotFoundError("updateTeamsApp", inputs["manifest-file"]));
            }
        }
    ...
    async validateTeamsApp(inputs) {
        ...
        if (!inputs["manifest-file"] && !inputs["package-file"]) {
            ...
            const defaultManifestPath = ManifestUtils_1.manifestUtils.getTeamsAppManifestPath(inputs.projectPath);
            if (!(await fs_extra_1.default.pathExists(defaultManifestPath))) {
                return teamsfx_api_1.err(new common_1.MissingRequiredInputError("package-file/manifest-file", "validateTeamsApp"));
            }
            inputs["manifest-file"] = defaultManifestPath;
        }
        if (inputs["manifest-file"]) {
