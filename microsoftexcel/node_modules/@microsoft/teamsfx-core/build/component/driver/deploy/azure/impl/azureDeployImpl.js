    ...
    async createAzureDeployConfig(azureResource, azureCredential) {
        ...
        try {
            ...
            if (token) {
                ...
                this.logger.info("Get AAD token failed. AAD Token is empty. Upload zip package through basic auth mode. Please check your Azure credential.");
            }
        }
        catch (e) {
            this.context.telemetryReporter.sendTelemetryErrorEvent("Get-Deploy-AAD-token-failed", {
        ...
        if (process.env["TEAMSFX_AAD_DEPLOY_ONLY"] === "true") {
            throw new error_1.GetPublishingCredentialsError(azureResource.instanceId, azureResource.resourceGroupName, new Error("Get AAD token failed."), this.helpLink);
        }
